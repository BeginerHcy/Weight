...
#define TIMEOUT		400		/* 接收命令超时时间, 单位ms */
#define NUM			1		/* 循环发送次数 */
...
void MODH_Send0FH(uint8_t _addr, uint16_t _reg, uint16_t _num, BOOL *_buf)
{
	...
	g_tModH.TxBuf[g_tModH.TxCount++] = _num >> 8;	/* 寄存器个数 高字节 */
	g_tModH.TxBuf[g_tModH.TxCount++] = _num;		/* 寄存器个数 低字节 */
	g_tModH.TxBuf[g_tModH.TxCount++] = _bytenum;	/* 数据字节数 */
...
void MODH_ReciveNew(uint8_t _data)
{
	...
	*/
	...
	timeout = 35000000 / HBAUD485;		/* 计算超时时间，单位us 35000 000*/
...
uint8_t MODBUS_02H(uint8_t slavAdr, uint16_t _reg, BOOL * varAdr, uint16_t _num)
{
	...
	switch(r02state){
		case 0:
			...
			time2 = bsp_GetRunTime();	/* 记录命令发送的时刻 */
			r02state = 1;
			break;
...
uint8_t MODBUS_03H(uint8_t slavAdr, uint16_t _reg, uint16_t * varAdr, uint16_t _num)
{
	...
	switch(r03state){
		case 0:
			...
			time3 = bsp_GetRunTime();	/* 记录命令发送的时刻 */
			r03state = 1;
			break;
...
uint8_t MODBUS_04H(uint8_t slavAdr, uint16_t _reg, uint16_t * varAdr, uint16_t _num)
{
	...
	switch(r04state){
		case 0:
			...
			time4 = bsp_GetRunTime();	/* 记录命令发送的时刻 */
			r04state = 1;
			break;
...
uint8_t MODBUS_0FH(uint8_t slavAdr, uint16_t _reg, BOOL * varAdr, uint16_t _num){
	...
	switch(r15state){
		...
		case 0:
			...
			time15 = bsp_GetRunTime();	/* 记录命令发送的时刻 */
			r15state = 1;
			break;
...
uint8_t MODBUS_10H(uint8_t slavAdr, uint16_t _reg, uint16_t * varAdr, uint16_t _num)
{
	...
	switch(r16state){
		...
		case 0:
			...
			time16 = bsp_GetRunTime();	/* 记录命令发送的时刻 */
			r16state = 1;
			break;
